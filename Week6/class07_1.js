...
class particleSystem {
	...
	gravityFallInit() {
		...
		const backWall = new THREE.Mesh(boxGeometry, metalMaterial);
		backWall.position.set(0, 0, -1.1);
		backWall.rotation.x = Math.PI/2;
		scene.add(backWall);
		
		const leftWall = new THREE.Mesh(boxGeometry, metalMaterial);
		leftWall.position.set(-1.1, 0, 0);
		leftWall.rotation.z = Math.PI/2;
		scene.add(leftWall);

		const rightWall = new THREE.Mesh(boxGeometry, metalMaterial);
		rightWall.position.set(1.1, 0, 0);
		rightWall.rotation.z = Math.PI/2;
		scene.add(rightWall);

		const frontWall = new THREE.Mesh(boxGeometry, metalMaterial);
		frontWall.position.set(0, 0, 1.1);
	...
	gravityFall() {
		...
		for (let i = 0; i < this.m_positions.length; i += 3) {
			...
			if (this.m_positions[i + 1] < -1.0 && Math.abs(this.m_positions[i]) < 1.0 && Math.abs(this.m_positions[i + 2]) < 1.0) {
				this.m_velocity[i + 1] = .6 * Math.abs(this.m_velocity[i + 1]);
			}
			//back
			if (this.m_positions[i + 2] < -1.0 && Math.abs(this.m_positions[i]) < 1.0 && Math.abs(this.m_positions[i + 1]) < 1.0) {
				this.m_velocity[i + 2] = 0.85 * Math.abs(this.m_velocity[i + 2]);
			}
			//left
			if (this.m_positions[i] < -1.0 && Math.abs(this.m_positions[i + 1]) < 1.0 && Math.abs(this.m_positions[i + 2]) < 1.0) {
				this.m_velocity[i] = 0.85 * Math.abs(this.m_velocity[i]);
			}
			//right
			if (this.m_positions[i] > 1.0 && Math.abs(this.m_positions[i + 1]) < 1.0 && Math.abs(this.m_positions[i + 2]) < 1.0) {
				this.m_velocity[i] = -0.85 * Math.abs(this.m_velocity[i]);
			...
			//front
			if (this.m_positions[i + 2] > 1.0 && Math.abs(this.m_positions[i + 1]) < 1.0 && Math.abs(this.m_positions[i]) < 1.0) {
				this.m_velocity[i + 2] = -0.85 * Math.abs(this.m_velocity[i + 2]);
			}
